...
if study == 'Ukraine':
    ...
    REGIONS = ['all']  #['Chornomorskyi-Автономна Республіка Крим'] #['all']  #["Khersons'ka", "Kharkivs'ka"] #['all']  # ["Khersons'ka"] # ['all'] #["Khersons'ka", "Kharkivs'ka"]
    COUNTRY = 'Ukraine'

    # I/O DETAILS
    dir_afis = dm_cst.dir_afis_Ukraine_gaul2
    DIR_OUT_FIGS = dm_cst.DIR_OUT_FIGS_NDVI1km_profiles_by_ASAP_admin_by_customAFI_yearly_map_clean_gual2
    ...
    if xdomain == 'time':
        ...
        DICT_XDOMAIN = {'x': 'time', 'START_YYYYMMDD': 20190101, 'END_YYYYMMDD': 20220411, #20220411,
...
if study == 'ZAeast':
    print('Study not yet implemented')
...
def main():
    ...
    features['NAME_LAT'] = features['NAME_LAT'] + '-' + features['NAME_OBL']
    # features = features[features['adm0_name'] == COUNTRY]
    if REGIONS[0] != 'all':
         features = features[features['NAME_LAT'].isin(REGIONS)]
    ...
    if DICT_XDOMAIN['x'] == 'time':
        ...
        fn_list = dm_cst.NDVI_FN_TEMPLATE_gdd.format('*')
    ...
    for yr in yearOfAFI:
        ...
        for index, feat in features.iterrows():
            ...
            afi_df = pd.concat([afi_df, pd.DataFrame([[feat['ID'], 'Ukraine', feat['NAME_LAT'],
    ...
    for dkd in dekads:
        ...
        tmp = features[['NAME_LAT','NAME_UA']].copy()
        if DICT_XDOMAIN['x'] == 'time':
            tmp['Date'] = tbx_util.date.dekade2date(dkd)
        elif DICT_XDOMAIN['x'] == 'gdd':
            tmp['Date'] = dkd
        tmp['Avg_NDVI'], tmp['SD_NVDI'],  tmp['nValid'], tmp['Area_from_afi'], tmp['asap1_id'] = avg, std, nValid, features['area_from_afi'], features['ID']
    ...
    for au in profile_df['NAME_LAT'].unique():
        print(au)
        fig = plt.figure(figsize=(10, 5))
        ax = fig.add_subplot(111)
        for yr in single_years:
            profile_df_yr = profile_df[profile_df['year']==yr]
            x = profile_df_yr[profile_df_yr['NAME_LAT']==au]['DateSameYear'].values
            #strAreaFromAfi = str('{:,}'.format(int(profile_df[profile_df['NAME_LAT'] == au]['Area_from_afi'].values[0] * km22ha/1000))) + ' kha'
            yrseries = yr
            if DICT_XDOMAIN['x'] == 'gdd':
                x = x.astype('float')
                yrseries = int(yr) + DICT_XDOMAIN['offsetYearOfInterest']
            strAreaFromAfi = str('{:,}'.format(int(profile_df_yr[profile_df_yr['NAME_LAT'] == au]['Area_from_afi'].values[0] * km22ha/1000))) + ' kha'
            yrseries = str(yrseries) + ' ' + strAreaFromAfi
            y = profile_df_yr[profile_df_yr['NAME_LAT']==au]['Avg_NDVI'].values
            e = profile_df_yr[profile_df_yr['NAME_LAT']==au]['SD_NVDI'].values
            # plot differently if it is the last year and gdd was requested
            if yr == single_years[-1] and DICT_XDOMAIN['x'] == 'gdd':
                nV = profile_df_yr[profile_df_yr['NAME_LAT'] == au]['nValid'].values * pix2km2 * km22ha/1000.0
                if nV[0] == 0:
                    nv_scaled = nV * 0.0
                else:
                    nv_scaled = nV/nV[0]
        ...
        if DICT_XDOMAIN['x'] == 'gdd':
            ...
            if nV[0] != 0:
                ...
                profile_df.loc[profile_df['NAME_LAT']==au,['lastGDDof05Area']] = lastGDDof05Area
        ...
        if ISYEARLYAFI == False and DICT_XDOMAIN['x'] == 'time':
            afi_au = features[features['NAME_LAT'] == au]['afi'].values[0] *100
    ...
    for au in profile_df['NAME_LAT'].unique():
        print('* '+au)
        profile_df_au = profile_df[profile_df['NAME_LAT'] == au]
        if profile_df_au['nValid'].iloc[0] != 0:
            ...
            row = profile_df_au[['asap1_id', 'adm0_name', 'adm1_name', 'Date', 'year', 'DateSameYear']].iloc[-1].tolist() + [np.NaN]
        df_anomalies = pd.concat([df_anomalies, pd.DataFrame([row], columns=colmns)], ignore_index=True)
    # map it
    map_df = features.merge(df_anomalies, how='left', left_on="ID", right_on="asap1_id")
    if DICT_XDOMAIN['x'] == 'time':
