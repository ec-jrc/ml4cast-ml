...
if __name__ == '__main__':
    ...
    for id in runIDs2rerun:
        ...
        if len(glob.glob(os.path.join(config.models_out_dir, myID + '*'))) == 0:
            bn = os.path.basename(glob.glob(os.path.join(config.models_spec_dir, myID + '*')))
            fn = glob.glob(os.path.join(config.models_spec_dir, myID + '*'))
            listSlow_bn.extend(bn)
            listSlow_fn.extend(fn)
    outDir = os.path.join(config.output_dir, 'listSlow')
