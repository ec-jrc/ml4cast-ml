...
def compute_wilcoxon(group, runs, configs, short_names):
    # ['noSF', 'ObsAsSF', 'SF']
    out = group.copy()
    out['wilcoxon'] = np.nan
    # get ref mres (ML_obs)
    ...
    if len(mRes_file) > 1:
        print('compute_wilcoxon, more than one file')
        sys.exit()
    ...
    test = d140_modelStats.paired_wilcoxon(err2_noSF, err2_ObsAsSF, alternative="greater", alpha=0.05, verbose=False)
    ...
    test = d140_modelStats.paired_wilcoxon(err2_noSF, err2_SF, alternative="greater", alpha=0.05, verbose=False)
    out.loc[(out['run_short_name'] == shortName) & (out['Estimator'] != 'PeakFPAR'), 'wilcoxon'] = test[1]
...
def nat_level_comparison(config, model_mRes_dfs, model_names, crop, month_inSeas, hue_order, colors, dir_out):
    ...
    for id, model in enumerate(model_names):
        ...
        # plot it
        if firstLoop:
            plt.plot(yLoo_true_nat['Year'], yLoo_true_nat['yLoo_true_weighted'], "--o", color='black', label="Observed")
            firstLoop = False
...
for crop in list_crop:
    ...
    # Wilcoxon
    df2 = (df2.groupby("forecast_time", group_keys=False).apply(compute_wilcoxon, runs=runs, configs=configs, short_names=short_names))
    # Figure
...
for crop in list_crop:
    ...
    for month_inSeas in months_inSeas:
        ...
        rmseNatprct, r2Nat = nat_level_comparison(config, model_mRes_dfs, model_names, crop, month_inSeas, hue_order, colors, dir_out)
